...
?>
<form action="lab4-show.php" method="get">
<h1>資料庫查詢系統</h1>
    ...
    $result = mysqli_query($link, $sql_query);
      while($row=mysqli_fetch_row($result)) {
          for($j=0; $j<mysqli_num_fields($result); $j++) {           
            ...
            if($type==$row[$j]){
              echo "<input type='radio' name='type' value='$row[$j]' checked>$row[$j]</br>";     
  ...
  <legend>Fields Selector</legend>
    ...
    <?php
      ...
      while ($meta = mysqli_fetch_field($result)) {
        echo "<input type='checkbox' name='datach[]' value='$meta->name' >";
        echo "<label> $meta->name</br>";
